...
function MarketRankingPanel({ room, command, busy }: { room: DashboardRoom; command: (type: string, extra?: Record<string, unknown>) => void; busy: boolean }) {
  ...
  return <Paper sx={{ p: 2.2, borderRadius: 3, border: '2px solid #b36f42', bgcolor: '#fffaf4', mb: 2 }}>
    <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'start', sm: 'center' }} spacing={1}>
      <Box>
        <Typography variant="overline" color="primary">MARKET RANKING</Typography>
    ...
    {draws.length > 0 ? <Grid container spacing={1.2} sx={{ mt: 1.5 }}>{draws.map((draw) => <Grid key={draw.playerId} size={{ xs: 12, sm: 6, md: 3 }}><Paper variant="outlined" sx={{ p: 1.2, bgcolor: '#fff' }}><Typography variant="caption" color="text.secondary">第 {draw.rank} 名</Typography><Typography variant="subtitle2" fontWeight={900}>{draw.playerName}</Typography><Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 1, flexWrap: 'wrap' }}>{types.filter((type) => draw.customerCounts[type] > 0).map((type) => <Box key={type} sx={{ display: 'inline-flex', alignItems: 'center', gap: .4 }}><CustomerTypeTokens type={type} count={draw.customerCounts[type]} size={14} /><Typography variant="caption">{typeLabels[type]} {draw.customerCounts[type]}</Typography></Box>)}{draw.total === 0 && <Typography variant="caption" color="text.secondary">袋中不足，抽到 0 位</Typography>}</Stack><Typography variant="caption" color="text.secondary">共抽到 {draw.total} 位</Typography></Paper></Grid>)}</Grid> : rankedPlayers.length > 0 && <Grid container spacing={1.2} sx={{ mt: 1.5 }}>{rankedPlayers.map((player, index) => <Grid key={player.name} size={{ xs: 6, sm: 3 }}><Paper variant="outlined" sx={{ p: 1.4, textAlign: 'center', bgcolor: '#fff' }}><Typography variant="caption" color="text.secondary">第 {index + 1} 名</Typography><Typography variant="subtitle2" fontWeight={900} noWrap>{player.name}</Typography><Typography variant="h4" fontWeight={900} color="primary">{player.count}</Typography><Typography variant="caption">位顧客</Typography></Paper></Grid>)}</Grid>}
  </Paper>
}
...
function PlayerSummaryBar({ room }: { room: DashboardRoom }) {
  const me = room.me
  const metrics = [['餘額', `$${me?.cash ?? 0} 萬`], ['營收', `$${me?.revenue ?? 0} 萬`], ['饕客滿意', `${me?.gourmetSatisfaction ?? 0}`], ['一般客滿意', `${me?.regularSatisfaction ?? 0}`], ['貸款', `${me?.loans ?? 0} / 6`], ['品牌', `${me?.brandAwareness ?? 0}`], ['產品', `${me?.products ?? 0}`], ['價值', `${me?.values ?? 0}`], ['資源', `${me?.resources ?? 0}`]]
