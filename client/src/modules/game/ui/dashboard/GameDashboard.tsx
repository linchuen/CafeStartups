...
function MarketRankingPanel({ room, command, busy }: { room: DashboardRoom; command: (type: string, extra?: Record<string, unknown>) => void; busy: boolean }) {
  ...
  const draws = [...(room.marketDraws ?? [])].sort((left, right) => left.rank - right.rank)
  const typeLabels = { gourmet: '饕客', regular: '一般客', difficult: '困難客' }
  const bag = room.marketBag ?? { gourmet: 0, regular: 0, difficult: 0 }
  return <Paper sx={{ p: 2.2, borderRadius: 3, border: '2px solid #b36f42', bgcolor: '#fffaf4', mb: 2 }}><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'start', sm: 'center' }} spacing={1}><Box><Typography variant="overline" color="primary">MARKET RANKING</Typography><Typography variant="h6" fontWeight={900}>市場排名與抽取結果</Typography><Typography variant="body2" color="text.secondary">6 回合完成後，最後一張牌的市場變動才會放入袋中，再依排名抽取。</Typography></Box><Button variant="contained" onClick={() => command(ranking.length ? 'RESOLVE_LEARNING' : 'DRAW_MARKET')} disabled={busy}>{ranking.length ? '確認排名並結算' : '抽取市場袋顧客數'}</Button></Stack><Stack direction="row" spacing={1} sx={{ mt: 1.5, flexWrap: 'wrap' }}><Typography variant="body2" fontWeight={900}>市場袋剩餘：</Typography>{(['gourmet', 'regular', 'difficult'] as const).map((type) => <Box key={type} sx={{ display: 'inline-flex', alignItems: 'center', gap: .4 }}>{bag[type] > 0 && <CustomerTypeTokens type={type} count={bag[type]} size={14} />}<Typography variant="caption">{typeLabels[type]} {bag[type]}</Typography></Box>)}</Stack>{draws.length > 0 ? <Grid container spacing={1.2} sx={{ mt: 1.5 }}>{draws.map((draw) => <Grid key={draw.playerId} size={{ xs: 12, sm: 6, md: 3 }}><Paper variant="outlined" sx={{ p: 1.2, bgcolor: '#fff' }}><Stack direction="row" justifyContent="space-between" alignItems="center"><Typography variant="caption" color="text.secondary">第 {draw.rank} 名</Typography><Typography variant="subtitle2" fontWeight={900}>{draw.playerName}</Typography></Stack><Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 1, flexWrap: 'wrap' }}>{(['gourmet', 'regular', 'difficult'] as const).filter((type) => draw.customerCounts[type] > 0).map((type) => <Box key={type} sx={{ display: 'inline-flex', alignItems: 'center', gap: .4 }}><CustomerTypeTokens type={type} count={draw.customerCounts[type]} size={14} /><Typography variant="caption">{typeLabels[type]} {draw.customerCounts[type]}</Typography></Box>)}{draw.total === 0 && <Typography variant="caption" color="text.secondary">袋中不足，抽到 0 位</Typography>}</Stack><Typography variant="caption" color="text.secondary">共抽到 {draw.total} 位</Typography></Paper></Grid>)}</Grid> : ranking.length > 0 && <Grid container spacing={1.2} sx={{ mt: 1.5 }}>{ranking.map((count, index) => <Grid key={index} size={{ xs: 6, sm: 3 }}><Paper variant="outlined" sx={{ p: 1.4, textAlign: 'center', bgcolor: '#fff' }}><Typography variant="caption" color="text.secondary">第 {index + 1} 名</Typography><Typography variant="h4" fontWeight={900} color="primary">{count}</Typography><Typography variant="caption">位顧客</Typography></Paper></Grid>)}</Grid>}</Paper>
}
