...
export function CardCost({ card, color, compact = false }: { card: PlayerCard; color?: string; compact?: boolean }) {
  ...
  return <Stack direction="row" spacing={compact ? .35 : .55} alignItems="center" sx={{ minWidth: 0, flexWrap: compact ? 'wrap' : 'nowrap' }}>
    ...
    {icons.length > 0 && <Box component="span" role="img" aria-label={`資源需求 ${icons.length} 個`} sx={{ display: 'inline-flex', alignItems: 'center', flexWrap: 'wrap', gap: compact ? .15 : .3, ml: compact ? 0 : .25 }}>
