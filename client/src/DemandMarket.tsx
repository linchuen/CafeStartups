...
import { AttachMoney, Cake, Coffee, Restaurant, Spa, VolunteerActivism } from '@mui/icons-material'

export type DemandCardData = { id: string; row: 'ordinary' | 'advanced'; quantity: 1 | 2; icons: string[]; source?: string }

const DEMAND_TYPES = [
  { id: 'coffee', label: '咖啡', icon: Coffee, color: '#b87932', pale: '#f4e4ce' },
  { id: 'dessert', label: '甜點', icon: Cake, color: '#d18b3d', pale: '#f7e6c8' },
  { id: 'beans', label: '咖啡豆', icon: Spa, color: '#8b6845', pale: '#ece1d2' },
  { id: 'taste', label: '可口美味', icon: Restaurant, color: '#bd5c50', pale: '#f3dcd7' },
  { id: 'service', label: '貼心服務', icon: VolunteerActivism, color: '#b94e55', pale: '#f3d8db' },
  { id: 'price', label: '合理價格', icon: AttachMoney, color: '#ad4e4b', pale: '#f1d9d4' },
] as const
...
export function DemandCard({ card, revealed }: { card: DemandCardData; revealed: boolean }) {
  ...
  return <div className={`demand-card demand-card-face ${card.row}`} style={style}><div className="demand-card-heading"><span>{card.row === 'ordinary' ? '\u666e\u901a\u9700\u6c42' : '\u9032\u968e\u9700\u6c42'}</span></div><div className="demand-card-icons">{card.icons.map((icon) => { const Icon = iconOf(icon).icon; return <span key={icon} title={iconOf(icon).label}><Icon fontSize="inherit" /></span> })}</div><div className="demand-card-labels">{card.icons.map((icon) => <small key={icon}>{iconOf(icon).label}</small>)}</div></div>
}
