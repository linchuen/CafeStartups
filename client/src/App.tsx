...
type GameState = { id: string; roomCode: string; status: string; seed: string; gameVersion: number; period: number; phase: string; round: number; demandBoard?: Record<string, number>; players: Player[]; me?: { id: string; hand: Card[]; tableau: Card[]; discardCount: number; cash: number; loans: number; customers?: { kind: string; demand: string; unitPrice: number; count: number }[]; revenue?: number; score?: number; selectedKPIs?: string[]; brandAwareness?: number; products?: number; values?: number; resources?: number } }
...
export function App() {
  ...
  return <main className="app-shell">
    ...
    {screen === 'lobby' && room && <Lobby room={room} host={host} busy={busy} allReady={allReady} kpis={kpis} setKpis={setKpis} toggleReady={toggleReady} startGame={startGame} leave={leave} error={error} />}
