...
type Player = { id: string; displayName: string; ready: boolean; cash: number; loans: number; handCount: number }
...
function Lobby(props: { room: GameState; host: boolean; busy: boolean; allReady: boolean; toggleReady: () => void; startGame: () => void; leave: () => void; error: string }) {
  return <section className="lobby-page layout-grid"><div className="panel room-card"><p className="eyebrow">WAITING ROOM</p><h1>{props.room.roomCode}</h1><p className="muted">把這組代碼分享給你的夥伴</p><div className="player-list">{props.room.players.map((player, index) => <div className="player-row" key={player.id}><span className="avatar">{player.displayName.slice(0, 1)}</span><span><strong>{player.displayName}</strong>{index === 0 && <small>房主</small>}</span><span className={player.ready ? 'ready' : 'waiting'}>{player.ready ? '已準備' : '等待中'}</span></div>)}{Array.from({ length: 4 - props.room.players.length }).map((_, i) => <div className="player-row empty" key={i}><span className="avatar">·</span><span>等待玩家加入</span></div>)}</div><div className="lobby-actions"><button className="secondary" onClick={props.leave}>離開</button><button className="primary" onClick={props.toggleReady} disabled={props.busy}>{props.room.players.find((p) => p.id === props.room.me?.id)?.ready ? '取消準備' : '準備完成'}</button>{props.host && <button className="accent" onClick={props.startGame} disabled={props.busy || !props.allReady || props.room.players.length < 2}>開始遊戲</button>}</div>{props.host && !props.allReady && <p className="hint">所有玩家準備完成後，房主才能開始遊戲。</p>}{props.error && <p className="error">{props.error}</p>}</div><aside className="rules-card"><p className="eyebrow">HOW TO PLAY</p><h2>一場實驗，六次選擇</h2><p>每個時期會拿到 7 張管理牌。選牌、傳牌，最後把你的策略打進咖啡館。</p><ol><li>設定你的關鍵指標</li><li>每回合選一張牌並傳遞</li><li>打出策略或棄牌換取現金</li></ol></aside></section>
}
