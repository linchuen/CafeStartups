...
使用 Go + React 將《Café Startups》製作成可在瀏覽器進行的桌遊 MVP。產品首先要能讓 2–4 位玩家在同一個房間完成一局三時期遊戲，並清楚呈現每個玩家的手牌、咖啡館牌組、現金、貸款、顧客與分數。

規則來源：工作區根目錄的 `Cafe Startups 規則書_2025.pdf`。若本文件與規則書衝突，以規則書為準；若 MVP 為了可玩性採用簡化規則，必須在需求文件與 UI 中標示為「MVP 簡化」。

## 2. Agent 工作原則

1. 先讀 `Agent.md` 與 `docs/MVP-階段式需求.md`，再修改程式。
...
8. 不為了 MVP 預先加入登入、支付、社交、原生 App、AI 對手或完整工作坊功能。
...
- 遊戲有 3 個時期：試營運、正式營運、擴大營運；每期有假設、實驗、學習三階段。
- 初始玩家取得 150 萬元、1 張合夥人卡與 1 張創始店卡。
- 每期發 7 張經營管理卡；實驗階段進行 6 回合的選牌、傳牌、同步行動，最後 1 張進入中央覆蓋區。
